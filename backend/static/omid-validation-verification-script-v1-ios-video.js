...
function fireEvent(event) {
    var params = serialize(event),
        url = "http://10.0.2.2:8002/events/sendmessage?" + params;
    fireURL(url);
}
...
(function(omidGlobal) {
    ...
    fireURL("http://10.0.2.2:8002/events/sendmessage?version=" + key);

    if (!verificationClient.isSupported()) {
        fireURL("http://10.0.2.2:8002/events/sendmessage?supported=no");
    ...
    fireURL("http://10.0.2.2:8002/events/sendmessage?supported=yes");
